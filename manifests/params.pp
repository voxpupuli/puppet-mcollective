...
class mcollective::params {
  ...
  $nrpe_dir_real = $operatingsystem ? {
    /(?i-mx:centos|fedora|redhat|oel|slc|scientific)/ => '/etc/nrpe.d',
  ...
  $mc_libdir = $operatingsystem ? {
    ...
    /(?i-mx:centos|fedora|redhat|slc|scientific)/ => '/usr/libexec/mcollective',
  }

  $mc_service_start = $operatingsystem ? {
    /(?i-mx:ubuntu|debian)/        => '/etc/init.d/mcollective start',
    /(?i-mx:centos|fedora|redhat|slc|scientific)/ => '/sbin/service mcollective start',
  }

  $mc_service_stop = $operatingsystem ? {
    /(?i-mx:ubuntu|debian)/        => '/etc/init.d/mcollective stop',
    /(?i-mx:centos|fedora|redhat|slc|scientific)/ => '/sbin/service mcollective stop',
  }
